...
- `panic`은 정말로 필요한 곳(`init`, 할당 불가능한 리소스) 외에는 사용하지 마라. 일반 비즈니스 로직에서는 `error` 반환 체인을 사용해라.
- `_ = foo()` 식의 에러 무시는 Go를 쓰는 의미를 스스로 부정하는 행위다. Go는 모든 에러를 명시적으로 처리하게 강제하는데, 그걸 무시할 바에는 에러가 자동으로 전파되는 JavaScript의 `try/catch`를 쓰러 가라. 에러는 반드시 로깅하거나 호출자에게 전파해라.
